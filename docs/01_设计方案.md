...
基于 ChatGPT 分享对话《Unity敌人AI设计方案》整理。该对话从基础 FSM 方案逐步演进为「xNode 状态机 + 技能时间轴（SkillTimeline，UI Toolkit 编辑器）+ ScriptableObject 配置」的完整架构。

> **实现现状（2026-08-22）**：本设计已全部落地，见 `Assets/Game/README.md` 与 [04_开发进度.md](./04_开发进度.md)。实际实现与设计的主要差异：
> - 状态切换条件从"代码硬编码"演进为「xNode 条件节点可视化连线」（`TransitionConditionNode`）
> - 玩家升级为**双武器**：左键近战（剑）/ 右键远程（手枪），带冷却 UI
> - **编辑器迁移（2026-08-22）**：xNode 已替换为 **Unity GraphView**（`Assets/Game/AI/Editor/GraphView/`），运行时改为纯序列化数据模型（`EnemyStateGraph` + `EnemyStateMachineCompiler`），逻辑与配置完全迁移，测试 EditMode 10/10、PlayMode 8/8 通过。
> - 新增运行时警戒范围可视化、程序化角色/武器模型、攻击动画、多敌人（近战 x2 + 远程 x1）
...
flowchart TD
    ...
    Selector --> Skill
    Skill --> Timeline["SkillTimelinePlayer"]
    Timeline --> Clips["技能 Clips"]
    Clips --> Combat["通用战斗系统"]
...
| SkillConfig | 冷却、距离、目标类型、SkillTimeline 时间轴资源 |
| SkillTimeline | 动画、前摇、伤害帧、位移、子弹、特效、音效 |
| SkillRuntime | 当前冷却、目标、是否施法、已命中对象 |

> 避免多处重复配置同一参数：攻击前摇既然由技能时间轴决定，就不放进 `WeaponConfig`。
...
- `CharacterSkillController`：`TryCast(skill, request)` 校验（非空、非施法中、非冷却、范围内）→ 锁移动 → `SkillTimelinePlayer.Play(context, callback)` → `FinishSkill`（解锁移动、记冷却）；`Interrupt()` 强制打断
- `SkillTimelinePlayer` 作为适配层隔离时间轴运行时 API（原 Slate 适配层迁移，Slate 已删除）
- 每个角色必须拥有独立时间轴播放（播放位置、目标、命中记录独立）
...
- 伤害逻辑需限定 `if (!Application.isPlaying) return;`（Clip 通过 `SkillClipContext.CanResolve` 统一保护）
...
- F1 开关玩家无敌模式（`PlayerInvincibilityDebug`）
...
│   ├── Skills          SkillConfig.cs / CharacterSkillController.cs / SkillExecutionContext.cs / SkillCastRequest.cs / SkillTimelinePlayer.cs / SkillSelector.cs
│   ├── Timeline        SkillTimeline.cs / SkillTimelineClip.cs / SkillClipContext.cs / SkillTrackType.cs
│   ├── Timeline/Clips  FaceTargetClip.cs / LockMovementClip.cs / DashClip.cs / MeleeHitWindowClip.cs / SpawnProjectileClip.cs / ...
│   └── Projectiles     Projectile.cs（弹丸池未实现，按需扩展）
